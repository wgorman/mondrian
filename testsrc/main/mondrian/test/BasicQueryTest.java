...
public class BasicQueryTest extends FoodMartTestCase {
    ...
    public void testLargeDimQuery() {  
        ...
        testContext.assertQueryReturns(
            "WITH\n"
            + "     SET [Store IDs Fullset] AS 'Filter({AddCalculatedMembers([Store IDs].[All Store IDs].Children)}, (Count(CrossJoin({[Store IDs].CurrentMember},CrossJoin({AddCalculatedMembers({[Customer IDs].[All Customer IDs].Children, [Customer IDs].[All Customer IDs]})},{[Measures].[Unit Sales]})), ExcludeEmpty)))'\n"
            ...
            + "     SET [Synthetic Members Set Store IDs] AS '{  }'\n"
            + "     SET [Synthetic Members Set Customer IDs] AS '{  }'\n"
            + "     MEMBER [Measures].[Empty measure] AS 'null'\n"
            + "SELECT\n"
            + "     {[Store IDs Fullset], Ascendants([Store IDs].[All Store IDs])} ON COLUMNS,\n"
