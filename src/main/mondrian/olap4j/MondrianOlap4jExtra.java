...
class MondrianOlap4jExtra implements XmlaHandler.XmlaExtra {
    ...
    public Member checkReplaceMemberOrdinal(Member member) throws OlapException
    {
        ...
        // BaseRolapMember for the same member, one will be used for xmla
        // queries and only the other gets the ordinal updated.
        ...
        if (member.getOrdinal() == -1) {
            ...
            final mondrian.olap.SchemaReader schemaReader =
                olap4jMember.olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData
                    .olap4jConnection.getMondrianConnection().getSchemaReader()
                    .withLocus();
            MondrianOlap4jMember oMember =
                reLookupMember(schemaReader, olap4jMember);
    ...
    private MondrianOlap4jMember reLookupMember(
        SchemaReader schemaReader,
        MondrianOlap4jMember olap4jMember)
    {
        RolapMember seedMember =
            RolapUtil.strip((RolapMember) olap4jMember.member);
        if (seedMember.getParentMember() != null) {
            List<Id.Segment> uniqueName =
                Util.parseIdentifier(seedMember.getUniqueName());
            OlapElement element =
                schemaReader.getElementChild(
                    seedMember.getParentMember(),
                    uniqueName.get(uniqueName.size() -1));
            if (element != null && element instanceof RolapMember) {
                return
                    ...
                    .olap4jConnection.toOlap4j((RolapMember) element);
            }
        }
        return olap4jMember;
    }
    ...
    {
          return ((MondrianOlap4jConnection) connection).prepareOlapStatement(
              mdx,
              parameters);
    }
}
